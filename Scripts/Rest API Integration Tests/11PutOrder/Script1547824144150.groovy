...
responseOrder = WS.sendRequestAndVerify(findTestObject('REST API/Add order'))

def jsonSlurper = new JsonSlurper()

def order = jsonSlurper.parseText(responseOrder.getResponseText())

WS.sendRequestAndVerify(findTestObject('REST API/Put order', [('orderId') : "$order.id", ('state') : 'In realization']))

response = WS.sendRequestAndVerify(findTestObject('REST API/Get order', [('orderId') : "$order.id"]))

WS.sendRequestAndVerify(findTestObject('REST API/Delete order', [('orderId') : "$order.id"]))
