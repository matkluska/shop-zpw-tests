...
WS.sendRequestAndVerify(findTestObject('REST API/Add product', [('productName') : 'Produkt_testowy6']))
...
WS.sendRequestAndVerify(findTestObject('REST API/Put product', [('productId') : "$p6.id", ('name') : "$p6.name", ('description') : 'opis_edytowany',
	('products_quantity') : 10, ('price'): 12, ('category'): 'sport']))

def productResponse = WS.sendRequestAndVerify(findTestObject('REST API/Get product', [('productId') : "$p6.id"]))
...
WS.sendRequestAndVerify(findTestObject('REST API/Delete product', [('productId') : "$p6.id"]))