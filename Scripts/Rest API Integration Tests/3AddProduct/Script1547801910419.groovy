...
WS.sendRequest(findTestObject('REST API/Add product', [('productName') : 'Produkt_testowy3']))

response = WS.sendRequest(findTestObject('REST API/Get products', [('productsNames') : []]))

WS.sendRequest(findTestObject('REST API/Delete product', [('productId') : '']))
...
WS.sendRequestAndVerify(findTestObject('REST API/Delete product', [('productId') : "${p3.id}"]))
